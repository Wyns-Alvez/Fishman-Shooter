...
cave1_img = pygame.image.load('assets/Background/cave1.png').convert_alpha()
cave2_img = pygame.image.load('assets/Background/cave2.png').convert_alpha()
cave3_img = pygame.image.load('assets/Background/cave3.png').convert_alpha()
cave4_img = pygame.image.load('assets/Background/cave4.png').convert_alpha()
cave5_img = pygame.image.load('assets/Background/cave5.png').convert_alpha()
cave6_img = pygame.image.load('assets/Background/cave6.png').convert_alpha()
...
bullet_img2 = pygame.image.load('assets/bullet/bullet2.png').convert_alpha()
...
def draw_bg2():
    ...
    width = 12300
    for x in range(6):
        screen.blit(cave1_img, ((x * width) - bg_scroll * 0.1, 0))
        screen.blit(cave2_img, ((x * width) - bg_scroll * 0.16, SCREEN_HEIGHT - cave2_img .get_height() - 200))
        screen.blit(cave3_img, ((x * width) - bg_scroll * 0.16, SCREEN_HEIGHT - cave3_img.get_height() - 150))
        screen.blit(cave4_img, ((x * width) - bg_scroll * 0.16, SCREEN_HEIGHT - cave4_img.get_height() - -70))
        screen.blit(cave5_img, ((x * width) - bg_scroll * 0.16, SCREEN_HEIGHT - cave5_img.get_height() - -70))
        screen.blit(cave6_img, ((x * width) - bg_scroll * 0.1, 0))
...
##                    self.kill()
        ...
        if self.char_type == 'player':
            if (self.rect.right > SCREEN_WIDTH - SCROLL_THRESH and bg_scroll < (93 * TILE_SIZE) - SCREEN_WIDTH)\
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 95:
                        ...
                        exit = Exit(img, x * TILE_SIZE, y * TILE_SIZE)
                        exit_group.add(exit)
                    elif tile == 111:#platformer horizontal
                        platform = Platform(img, x * TILE_SIZE, y * TILE_SIZE, 0, 1)
                        platform_group.add(platform)
                    elif tile == 112:#platformer vertical
                        platform = Platform(img, x * TILE_SIZE, y * TILE_SIZE, 1, 0)
                        platform_group.add(platform)
                    elif tile == 113:#create enemies2
                        enemy2 = Enemy2('enemy2', x * TILE_SIZE, y * TILE_SIZE, 1.65, 2, 20, 0)
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        #load all images for the players
        animation_types = ['idle', 'death','walk','attack']
        for animation in animation_types:
            #reset temporary list of images
            temp_list = []
            #count number of files in the folder
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            self.shoot_cooldown = 20
            bullet = Bullet2(self.rect.centerx + (0.8 * self.rect.size[0] * self.direction), self.rect.centery, self.direction)
            bullet_group.add(bullet)
            #reduce ammo
            self.ammo -= 1
            shot_fx.play()
...
class Enemy2(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        #load all images for the players
        animation_types = ['idle', 'death','walk']
        for animation in animation_types:
            #reset temporary list of images
            temp_list = []
            #count number of files in the folder
